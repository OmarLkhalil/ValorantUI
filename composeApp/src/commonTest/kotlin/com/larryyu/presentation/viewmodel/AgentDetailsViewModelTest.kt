...
class AgentDetailsViewModelTest {
    ...
    fun setup() {
        ...
        mockAgentDetailsUseCase = FakeAgentDetailsUseCase()
        viewModel = AgentDetailsViewModel(mockAgentDetailsUseCase)
    }
    @AfterTest
    fun tearDown() {
    ...
    fun `initial state should be empty agent details`() = runTest {
        ...
        assertTrue(initialState.agentDetails.displayName?.isEmpty() ?: true, "Display name should be empty")
    ...
    fun `fetch agent details should load successfully`() = runTest {
        ...
        assertEquals("Jett", state.agentDetails.displayName, "Agent name should match")
        assertEquals(agentId, state.agentDetails.uuid, "Agent ID should match")
    ...
    fun `fetch different agents should update state correctly`() = runTest {
        ...
        assertEquals("Phoenix", secondState.agentDetails.displayName)
        assertNotEquals(firstState.agentDetails.uuid, secondState.agentDetails.uuid)
    }
    @Test
    fun `agent details with abilities should load correctly`() = runTest {
        ...
        advanceUntilIdle()
        val state = viewModel.state.value
        assertEquals(4, state.agentDetails.abilities?.size ?: 0, "Should have 4 abilities")
        assertTrue(
            state.agentDetails.abilities?.any { it.displayName == "Cloudburst" } == true,
    ...
    fun `rapid consecutive fetches should handle correctly`() = runTest {
        ...
        assertEquals("Test Agent", state.agentDetails.displayName)
    }
    @Test
    fun `error recovery should work on retry`() = runTest {
        ...
        assertEquals("Recovered Agent", successState.agentDetails.displayName)
    }
    @Test
    fun `agent details with null abilities should handle gracefully`() = runTest {
        ...
        advanceUntilIdle()
        val state = viewModel.state.value
        assertEquals("Agent", state.agentDetails.displayName)
        assertTrue(state.agentDetails.abilities == null || state.agentDetails.abilities.isEmpty())
    }
    private fun createMockAgentDetails(
        ...
        description = "Test description for $displayName",
...
class FakeAgentsRepoForAgentDetails : com.larryyu.domain.repository.AgentsRepo {
    ...
    override suspend fun getAgentsFromLocalDatabase() = emptyList<com.larryyu.domain.model.AgentsModel>()
    override suspend fun insertAgentsToLocalDatabase(agents: List<com.larryyu.domain.model.AgentsModel>) {}
}
